...
CREATE TABLE IF NOT EXISTS webhooks (
    id TEXT NOT NULL PRIMARY KEY,
    name TEXT NOT NULL,
    content_type TEXT NOT NULL DEFAULT 'application/json' CHECK(content_type IN ('application/json', 'application/xml')),
    url TEXT NOT NULL,
    method TEXT NOT NULL DEFAULT 'POST' CHECK(method IN ('GET', 'PUT', 'PATCH', 'POST', 'DELETE')),
    created_at TEXT NOT NULL DEFAULT (strftime('%Y-%m-%dT%H:%M:%fZ', 'now')),
