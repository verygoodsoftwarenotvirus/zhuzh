...
WHERE user_sessions.session_token_id = sqlc.arg(session_token_id)
	...
	AND user_sessions.expires_at > strftime('%Y-%m-%dT%H:%M:%fZ', 'now');
...
SELECT
	...
	(
		...
		WHERE user_sessions.belongs_to_user = sqlc.arg(belongs_to_user)
			AND user_sessions.revoked_at IS NULL
			AND user_sessions.expires_at > strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
			AND user_sessions.created_at > COALESCE(sqlc.narg(created_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
			AND user_sessions.created_at < COALESCE(sqlc.narg(created_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
	) AS filtered_count,
	(
		SELECT COUNT(user_sessions.id)
		FROM user_sessions
