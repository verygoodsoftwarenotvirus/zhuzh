...
func (q *Queries) CheckServiceSettingConfigurationExistence(ctx context.Context, db DBTX, id string) (string, error) {
...
type GetServiceSettingConfigurationByIDRow struct {
	...
	ServiceSettingType          string
	ServiceSettingDescription   string
	ServiceSettingDefaultValue  *string
	ServiceSettingEnumeration   string
	ServiceSettingAdminsOnly    bool
	ServiceSettingCreatedAt     string
	ServiceSettingLastUpdatedAt *string
	ServiceSettingArchivedAt    *string
	BelongsToUser               string
	BelongsToAccount            string
	CreatedAt                   string
...
SELECT
	...
	(
		...
		WHERE service_setting_configurations.archived_at IS NULL
			AND
			service_setting_configurations.created_at > COALESCE(?1, datetime('now', '-999 years'))
			AND service_setting_configurations.created_at < COALESCE(?2, datetime('now', '+999 years'))
			AND (
				service_setting_configurations.last_updated_at IS NULL
				OR service_setting_configurations.last_updated_at > COALESCE(?3, datetime('now', '-999 years'))
			)
			AND (
				service_setting_configurations.last_updated_at IS NULL
				OR service_setting_configurations.last_updated_at < COALESCE(?4, datetime('now', '+999 years'))
			)
			AND service_setting_configurations.belongs_to_account = ?5
	) AS filtered_count,
	(
		SELECT COUNT(service_setting_configurations.id)
		FROM service_setting_configurations
...
WHERE service_settings.archived_at IS NULL
	AND service_setting_configurations.archived_at IS NULL
	AND service_setting_configurations.belongs_to_account = ?5
	AND service_setting_configurations.created_at > COALESCE(?1, datetime('now', '-999 years'))
	AND service_setting_configurations.created_at < COALESCE(?2, datetime('now', '+999 years'))
	AND (
		service_setting_configurations.last_updated_at IS NULL
		OR service_setting_configurations.last_updated_at > COALESCE(?4, datetime('now', '-999 years'))
	...
	AND service_setting_configurations.id > COALESCE(?6, '')
...
SELECT
	...
	(
		...
		WHERE service_setting_configurations.archived_at IS NULL
			...
			AND service_setting_configurations.belongs_to_user = ?5
	) AS filtered_count,
	(
		SELECT COUNT(service_setting_configurations.id)
		FROM service_setting_configurations
