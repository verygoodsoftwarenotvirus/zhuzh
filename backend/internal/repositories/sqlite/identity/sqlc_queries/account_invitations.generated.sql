...
WHERE account_invitations.archived_at IS NULL
	AND account_invitations.expires_at > strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
...
SELECT
	...
	(
		...
		WHERE account_invitations.archived_at IS NULL
			AND
			account_invitations.created_at > COALESCE(sqlc.narg(created_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
			AND account_invitations.created_at < COALESCE(sqlc.narg(created_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
			AND (
				account_invitations.last_updated_at IS NULL
				OR account_invitations.last_updated_at > COALESCE(sqlc.narg(updated_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
			)
			AND (
				account_invitations.last_updated_at IS NULL
				OR account_invitations.last_updated_at < COALESCE(sqlc.narg(updated_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
			)
	) AS filtered_count,
	(
		SELECT COUNT(account_invitations.id)
...
WHERE account_invitations.archived_at IS NULL
	...
	AND account_invitations.created_at > COALESCE(sqlc.narg(created_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
	AND account_invitations.created_at < COALESCE(sqlc.narg(created_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
	AND (
		account_invitations.last_updated_at IS NULL
		OR account_invitations.last_updated_at > COALESCE(sqlc.narg(updated_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
