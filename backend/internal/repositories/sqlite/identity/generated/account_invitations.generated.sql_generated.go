...
func (q *Queries) CheckAccountInvitationExistence(ctx context.Context, db DBTX, id string) (string, error) {
...
type GetAccountInvitationByAccountAndIDRow struct {
	...
	AccountLatitude                          *float64
	AccountLongitude                         *float64
	AccountLastPaymentProviderSyncOccurredAt *string
	AccountWebhookHmacSecret                 string
	AccountCreatedAt                         string
	AccountLastUpdatedAt                     *string
	AccountArchivedAt                        *string
	FromUser                                 string
	ToUser                                   *string
	UserID                                   string
	UserUsername                             string
	...
	UserPasswordLastChangedAt                *string
	UserRequiresPasswordChange               bool
	UserTwoFactorSecret                      string
	UserTwoFactorSecretVerifiedAt            *string
	UserUserAccountStatus                    string
	UserUserAccountStatusExplanation         string
	UserBirthday                             *string
	UserEmailAddressVerificationToken        *string
	UserEmailAddressVerifiedAt               *string
	UserFirstName                            string
	UserLastName                             string
	UserLastAcceptedTermsOfService           *string
	UserLastAcceptedPrivacyPolicy            *string
	UserLastIndexedAt                        *string
	UserCreatedAt                            string
	UserLastUpdatedAt                        *string
	UserArchivedAt                           *string
	UserAvatarID                             *string
	UserAvatarStoragePath                    *string
	UserAvatarMimeType                       *string
	UserAvatarCreatedAt                      *string
	UserAvatarLastUpdatedAt                  *string
	UserAvatarArchivedAt                     *string
	UserAvatarCreatedByUser                  *string
	ToName                                   string
	Note                                     string
	ToEmail                                  string
	Token                                    string
...
SELECT
	...
	(
		...
		WHERE account_invitations.archived_at IS NULL
			AND
			account_invitations.created_at > COALESCE(?1, datetime('now', '-999 years'))
			AND account_invitations.created_at < COALESCE(?2, datetime('now', '+999 years'))
			AND (
				account_invitations.last_updated_at IS NULL
				OR account_invitations.last_updated_at > COALESCE(?3, datetime('now', '-999 years'))
			)
			AND (
				account_invitations.last_updated_at IS NULL
				OR account_invitations.last_updated_at < COALESCE(?4, datetime('now', '+999 years'))
			)
	) AS filtered_count,
	(
		SELECT COUNT(account_invitations.id)
...
WHERE account_invitations.archived_at IS NULL
	...
	AND account_invitations.created_at > COALESCE(?1, datetime('now', '-999 years'))
	AND account_invitations.created_at < COALESCE(?2, datetime('now', '+999 years'))
	AND (
		account_invitations.last_updated_at IS NULL
		OR account_invitations.last_updated_at > COALESCE(?4, datetime('now', '-999 years'))
	...
	AND account_invitations.id > COALESCE(?7, '')
