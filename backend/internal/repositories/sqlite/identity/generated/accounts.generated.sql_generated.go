...
type GetAccountByIDWithMembershipsRow struct {
	...
	Latitude                          *float64
	Longitude                         *float64
	LastPaymentProviderSyncOccurredAt *string
	WebhookHmacSecret                 string
	CreatedAt                         string
	...
	MembershipLastUpdatedAt           *string
	MembershipArchivedAt              *string
}
...
SELECT
	...
	(
		...
		WHERE accounts.archived_at IS NULL
			AND
			accounts.created_at > COALESCE(?1, datetime('now', '-999 years'))
			AND accounts.created_at < COALESCE(?2, datetime('now', '+999 years'))
			AND (
				accounts.last_updated_at IS NULL
				OR accounts.last_updated_at > COALESCE(?3, datetime('now', '-999 years'))
			)
			AND (
				accounts.last_updated_at IS NULL
				OR accounts.last_updated_at < COALESCE(?4, datetime('now', '+999 years'))
			)
	) AS filtered_count,
	(
		SELECT COUNT(accounts.id)
		FROM accounts
		WHERE accounts.archived_at IS NULL
			AND account_user_memberships.belongs_to_user = ?5
	) AS total_count
FROM accounts
JOIN account_user_memberships ON account_user_memberships.belongs_to_account = accounts.id
WHERE accounts.archived_at IS NULL
	AND account_user_memberships.archived_at IS NULL
	AND accounts.created_at > COALESCE(?1, datetime('now', '-999 years'))
	AND accounts.created_at < COALESCE(?2, datetime('now', '+999 years'))
	AND (
		accounts.last_updated_at IS NULL
		OR accounts.last_updated_at > COALESCE(?4, datetime('now', '-999 years'))
