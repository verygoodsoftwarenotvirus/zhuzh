...
UPDATE users SET archived_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE archived_at IS NULL AND id = ?1
`
...
type GetAdminUserByUsernameRow struct {
	...
	FirstName                     string
	LastName                      string
	LastAcceptedTermsOfService    *string
	...
	AvatarArchivedAt              *string
	AvatarCreatedByUser           *string
}
...
func (q *Queries) GetEmailVerificationTokenByUserID(ctx context.Context, db DBTX, id string) (*string, error) {
	row := db.QueryRowContext(ctx, getEmailVerificationTokenByUserID, id)
	var email_address_verification_token *string
...
func (q *Queries) GetUserByEmailAddressVerificationToken(ctx context.Context, db DBTX, emailAddressVerificationToken *string) (*GetUserByEmailAddressVerificationTokenRow, error) {
...
WHERE users.archived_at IS NULL
	...
	OR users.last_indexed_at < datetime('now', '-24 hours')
`
...
SELECT
	...
	(
		...
		WHERE users.archived_at IS NULL
			AND
			users.created_at > COALESCE(?1, datetime('now', '-999 years'))
			AND users.created_at < COALESCE(?2, datetime('now', '+999 years'))
			AND (
				users.last_updated_at IS NULL
				OR users.last_updated_at > COALESCE(?3, datetime('now', '-999 years'))
			)
			AND (
				users.last_updated_at IS NULL
				OR users.last_updated_at < COALESCE(?4, datetime('now', '+999 years'))
			)
	) AS filtered_count,
	(
		SELECT COUNT(users.id)
...
FROM users
	...
	LEFT JOIN uploaded_media ON uploaded_media.id = user_avatars.uploaded_media_id AND uploaded_media.archived_at IS NULL
WHERE users.archived_at IS NULL
	AND users.created_at > COALESCE(?1, datetime('now', '-999 years'))
	AND users.created_at < COALESCE(?2, datetime('now', '+999 years'))
	AND (
		users.last_updated_at IS NULL
		OR users.last_updated_at > COALESCE(?4, datetime('now', '-999 years'))
...
SELECT
	...
	(
		...
		WHERE users.archived_at IS NULL
			...
			AND account_user_memberships.belongs_to_account = ?5
	) AS filtered_count,
	(
		SELECT COUNT(users.id)
		FROM users
...
UPDATE users SET last_indexed_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ?1 AND archived_at IS NULL
`
