...
// versions:
//   sqlc v1.25.0
// source: oauth2_clients.generated.sql
...
SELECT
	...
	(
		...
		FROM oauth2_clients
		WHERE oauth2_clients.archived_at IS NULL
			AND oauth2_clients.created_at > COALESCE(?1, datetime('now', '-999 years'))
			AND oauth2_clients.created_at < COALESCE(?2, datetime('now', '+999 years'))
			AND oauth2_clients.id > COALESCE(?3, '')
	) as filtered_count,
	(
