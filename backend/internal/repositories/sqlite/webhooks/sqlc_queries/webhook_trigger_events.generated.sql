...
SELECT
	...
	(
		...
		WHERE webhook_trigger_events.archived_at IS NULL
			AND
			webhook_trigger_events.created_at > COALESCE(sqlc.narg(created_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
			AND webhook_trigger_events.created_at < COALESCE(sqlc.narg(created_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
			AND (
				webhook_trigger_events.last_updated_at IS NULL
				OR webhook_trigger_events.last_updated_at > COALESCE(sqlc.narg(updated_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
			)
			AND (
				webhook_trigger_events.last_updated_at IS NULL
				OR webhook_trigger_events.last_updated_at < COALESCE(sqlc.narg(updated_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
			)
	) AS filtered_count,
	(
		SELECT COUNT(webhook_trigger_events.id)
...
FROM webhook_trigger_events
WHERE webhook_trigger_events.archived_at IS NULL
	AND webhook_trigger_events.created_at > COALESCE(sqlc.narg(created_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
	AND webhook_trigger_events.created_at < COALESCE(sqlc.narg(created_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
	AND (
		webhook_trigger_events.last_updated_at IS NULL
		OR webhook_trigger_events.last_updated_at > COALESCE(sqlc.narg(updated_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
