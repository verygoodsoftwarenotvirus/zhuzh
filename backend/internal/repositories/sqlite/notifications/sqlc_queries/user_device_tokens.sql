...
SELECT
	...
	(
		...
		WHERE
			...
			AND (sqlc.narg(platform_filter) IS NULL OR user_device_tokens.platform = sqlc.narg(platform_filter))
			AND user_device_tokens.created_at > COALESCE(sqlc.narg(created_after), datetime('now', '-999 years'))
			AND user_device_tokens.created_at < COALESCE(sqlc.narg(created_before), datetime('now', '+999 years'))
	) AS filtered_count,
	(
		SELECT COUNT(user_device_tokens.id)
		FROM user_device_tokens
