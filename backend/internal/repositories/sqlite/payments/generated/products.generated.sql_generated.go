...
UPDATE products SET archived_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now'), last_updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE archived_at IS NULL AND id = ?1
`
...
func (q *Queries) CheckProductExistence(ctx context.Context, db DBTX, id string) (string, error) {
...
func (q *Queries) GetProductByExternalID(ctx context.Context, db DBTX, externalProductID *string) (*Products, error) {
...
SELECT
	...
	(
		...
		WHERE products.archived_at IS NULL
			AND
			products.created_at > COALESCE(?1, datetime('now', '-999 years'))
			AND products.created_at < COALESCE(?2, datetime('now', '+999 years'))
			AND (
				products.last_updated_at IS NULL
				OR products.last_updated_at > COALESCE(?3, datetime('now', '-999 years'))
			)
			AND (
				products.last_updated_at IS NULL
				OR products.last_updated_at < COALESCE(?4, datetime('now', '+999 years'))
			)
	) AS filtered_count,
	(
		SELECT COUNT(products.id)
...
FROM products
WHERE products.archived_at IS NULL
	AND products.created_at > COALESCE(?1, datetime('now', '-999 years'))
	AND products.created_at < COALESCE(?2, datetime('now', '+999 years'))
	AND (
		products.last_updated_at IS NULL
		OR products.last_updated_at > COALESCE(?4, datetime('now', '-999 years'))
