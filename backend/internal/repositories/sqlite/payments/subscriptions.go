...
func (r *repository) CreateSubscription(ctx context.Context, input *payments.SubscriptionDatabaseCreationInput) (*payments.Subscription, error) {
	...
	arg := &generated.CreateSubscriptionParams{
		...
		ExternalSubscriptionID: stringPtrFromString(input.ExternalSubscriptionID),
		Status:                 input.Status,
		CurrentPeriodStart:     input.CurrentPeriodStart.Format(time.RFC3339Nano),
		CurrentPeriodEnd:       input.CurrentPeriodEnd.Format(time.RFC3339Nano),
	}
...
func (r *repository) GetSubscriptionByExternalID(ctx context.Context, externalID string) (*payments.Subscription, error) {
	...
	arg := stringPtrFromString(externalID)
...
func (r *repository) UpdateSubscription(ctx context.Context, sub *payments.Subscription) error {
	...
	arg := &generated.UpdateSubscriptionParams{
		ID:                     sub.ID,
		ExternalSubscriptionID: stringPtrFromString(sub.ExternalSubscriptionID),
		Status:                 sub.Status,
		CurrentPeriodStart:     sub.CurrentPeriodStart.Format(time.RFC3339Nano),
		CurrentPeriodEnd:       sub.CurrentPeriodEnd.Format(time.RFC3339Nano),
	}
...
func convertSubscriptionFromGenerated(m *generated.Subscriptions) *payments.Subscription {
	...
	return &payments.Subscription{
		...
		ExternalSubscriptionID: stringFromStringPtr(m.ExternalSubscriptionID),
		Status:                 m.Status,
		CurrentPeriodStart:     parseTime(m.CurrentPeriodStart),
		CurrentPeriodEnd:       parseTime(m.CurrentPeriodEnd),
...
func convertSubscriptionsResult(rows []*generated.GetSubscriptionsForAccountRow, filter *filtering.QueryFilter) *filtering.QueryFilteredResult[payments.Subscription] {
	...
	for _, row := range rows {
		data = append(data, &payments.Subscription{
			...
			ExternalSubscriptionID: stringFromStringPtr(row.ExternalSubscriptionID),
			Status:                 row.Status,
			CurrentPeriodStart:     parseTime(row.CurrentPeriodStart),
			CurrentPeriodEnd:       parseTime(row.CurrentPeriodEnd),
