...
SELECT
	...
	(
		...
		FROM audit_log_entries
		WHERE
			audit_log_entries.created_at > COALESCE(sqlc.narg(created_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
			AND audit_log_entries.created_at < COALESCE(sqlc.narg(created_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
...
FROM audit_log_entries
WHERE audit_log_entries.created_at > COALESCE(sqlc.narg(created_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
	AND audit_log_entries.created_at < COALESCE(sqlc.narg(created_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
	AND audit_log_entries.belongs_to_user = sqlc.arg(belongs_to_user)
...
SELECT
	...
	(
		...
		WHERE
			...
			AND audit_log_entries.created_at < COALESCE(sqlc.narg(created_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
			AND audit_log_entries.belongs_to_user = sqlc.arg(belongs_to_user)
			AND audit_log_entries.resource_type IN (sqlc.slice(resources))
	) AS filtered_count,
	(
		SELECT COUNT(audit_log_entries.id)
		FROM audit_log_entries
