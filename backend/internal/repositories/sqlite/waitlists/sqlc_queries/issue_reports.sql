...
SELECT
	...
	(
		...
		WHERE issue_reports.archived_at IS NULL
			AND
			issue_reports.created_at > COALESCE(sqlc.narg(created_after), datetime('now', '-999 years'))
			AND issue_reports.created_at < COALESCE(sqlc.narg(created_before), datetime('now', '+999 years'))
			AND (
				issue_reports.last_updated_at IS NULL
				OR issue_reports.last_updated_at > COALESCE(sqlc.narg(updated_before), datetime('now', '-999 years'))
			)
			AND (
				issue_reports.last_updated_at IS NULL
				OR issue_reports.last_updated_at < COALESCE(sqlc.narg(updated_after), datetime('now', '+999 years'))
			)
			AND (NOT COALESCE(sqlc.narg(include_archived), 0) OR issue_reports.archived_at = NULL)
	) AS filtered_count,
	(
		SELECT COUNT(issue_reports.id)
		FROM issue_reports
...
FROM issue_reports
WHERE issue_reports.archived_at IS NULL
	AND issue_reports.created_at > COALESCE(sqlc.narg(created_after), datetime('now', '-999 years'))
	AND issue_reports.created_at < COALESCE(sqlc.narg(created_before), datetime('now', '+999 years'))
	AND (
		issue_reports.last_updated_at IS NULL
		OR issue_reports.last_updated_at > COALESCE(sqlc.narg(updated_after), datetime('now', '-999 years'))
