...
SELECT EXISTS(
	...
	WHERE waitlists.archived_at IS NULL
		AND waitlists.id = sqlc.arg(id)
		AND waitlists.valid_until >= strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
);
...
SELECT
	...
	(
		...
		WHERE waitlists.archived_at IS NULL
			AND
			waitlists.created_at > COALESCE(sqlc.narg(created_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
			AND waitlists.created_at < COALESCE(sqlc.narg(created_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
			AND (
				waitlists.last_updated_at IS NULL
				OR waitlists.last_updated_at > COALESCE(sqlc.narg(updated_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
			)
			AND (
				waitlists.last_updated_at IS NULL
				OR waitlists.last_updated_at < COALESCE(sqlc.narg(updated_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
			)
	) AS filtered_count,
	(
		SELECT COUNT(waitlists.id)
...
FROM waitlists
WHERE waitlists.archived_at IS NULL
	AND waitlists.created_at > COALESCE(sqlc.narg(created_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
	AND waitlists.created_at < COALESCE(sqlc.narg(created_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
	AND (
		waitlists.last_updated_at IS NULL
		OR waitlists.last_updated_at > COALESCE(sqlc.narg(updated_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
