...
FROM uploaded_media
WHERE uploaded_media.archived_at IS NULL
	AND uploaded_media.id IN (sqlc.slice(ids));
...
SELECT
	...
	(
		...
		WHERE uploaded_media.archived_at IS NULL
			AND
			uploaded_media.created_at > COALESCE(sqlc.narg(created_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
			AND uploaded_media.created_at < COALESCE(sqlc.narg(created_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
			AND (
				uploaded_media.last_updated_at IS NULL
				OR uploaded_media.last_updated_at > COALESCE(sqlc.narg(updated_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
			)
			AND (
				uploaded_media.last_updated_at IS NULL
				OR uploaded_media.last_updated_at < COALESCE(sqlc.narg(updated_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
			)
...
WHERE uploaded_media.archived_at IS NULL
	AND uploaded_media.created_by_user = sqlc.arg(created_by_user)
	AND uploaded_media.created_at > COALESCE(sqlc.narg(created_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
	AND uploaded_media.created_at < COALESCE(sqlc.narg(created_before), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '+999 years'))
	AND (
		uploaded_media.last_updated_at IS NULL
		OR uploaded_media.last_updated_at > COALESCE(sqlc.narg(updated_after), strftime('%Y-%m-%dT%H:%M:%fZ', 'now', '-999 years'))
