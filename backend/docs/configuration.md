...
**Example**: If the JSON config sets `config.GRPCServer.Port` to `1234` and you run the application with `ZHUZH_GRPC_PORT=4321`, the service will use `4321`.
...
Env vars follow this convention: `ZHUZH_` + `PREFIX_` + `FIELD`.

- **Prefix**: `ZHUZH_` (see `config.EnvVarPrefix`)
...
For example, `config.GRPCServer.Port` maps to `ZHUZH_GRPC_PORT` because:
